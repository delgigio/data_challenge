...
We obtain a good recall of over 0.8 and an AUROC comparable to before. The undersampling was definitely needed as this is a way better model than the previous one. However we now have many false positives. We now check the PR curve to see if there are thresholds that result in a more balanced confusion matrix.


```{r}
levels(us_logreg_valPred$stroke)
```


```{r}
plot_pr_curve(us_logreg_valPred, 'stroke')
```
...
```
We have similar or even worse performance on the train set, our model is not overfitting, thus we don't need to regularize it. We will not try LASSO or Ridge regression.
...
The recall estimates of the logistic regression and random forest models are almost equivalent and the ROC curve is worse. We will coose the first model. Since we care a lot about explainability, we won't consider the random forest unless it shows clear advantages over the competitor, which isn't the case. 
...
```{r}
tb_train_high <- tb_train_groups %>% mutate(high=ifelse(groups == 'high',1,0))

tb_train_high <- tb_train_high %>% select(-groups)

tb_train_high
```

```{r}
tb_train_medium <- tb_train_groups %>% mutate(medium=ifelse(groups == 'medium',1,0))

tb_train_medium <- tb_train_medium %>% select(-groups)

tb_train_medium
```

```{r}
tb_train_low <- tb_train_groups %>% mutate(low=ifelse(groups == 'low',1,0))

tb_train_low <- tb_train_low %>% select(-groups)
...
```{r}
#grafico a violino distribuzione BMI
install.packages("dplyr")
library(dplyr)

data_filtered <- data %>%
  filter(!is.na(bmi) & work_type != "")


ggplot(data_filtered, aes(x = work_type, y = bmi, fill = work_type)) +
  geom_violin(trim = FALSE) +
  geom_boxplot(width = 0.1, fill = "white") +  # Aggiunge un boxplot al centro del violino
  labs(title = "Distribuzione del BMI per Tipo di Lavoro",
       x = "Work type",
       y = "BMI") +
  theme_minimal() +
  theme(legend.position = "none")
...
The violin plot shows the distribution of BMI (Body Mass Index) divided by job type: children, government job, never worked, private sector, and self-employed.

The chart reveals that children generally have a lower and more concentrated BMI, while government, private sector, and self-employed workers show greater variability and a higher average BMI. The "Never worked" category has a distribution similar to children but slightly higher.

In summary, the chart highlights how BMI varies significantly based on job type, with greater diversity and higher average values in adult worker groups compared to children and those who have never worked.


```{r}
library(ggplot2)

# Creare un esempio di dataframe
set.seed(123)
data_bubble <- data.frame(
  age = sample(20:80, 100, replace = TRUE),
  bmi = runif(100, 15, 40),
  prevalence = sample(c(0, 1), 100, replace = TRUE)
)

# Creare il grafico a bolle
p <- ggplot(data_bubble, aes(x = age, y = prevalence, size = bmi, color = factor(prevalence))) +
  geom_point(alpha = 0.7) +
  scale_size_continuous(range = c(3, 10)) +
  labs(title = "Grafico a Bolle dell'Ictus",
       x = "Eta",
       y = "Prevalenza di Ictus",
       size = "BMI",
       color = "Prevalenza") +
  theme_minimal()

# Forzare la visualizzazione del grafico
print(p)
```

First of all, we observe that the prevalence of stroke (indicated by blue bubbles) is clearly distinct from the non-prevalence (indicated by red bubbles). The blue bubbles are concentrated exclusively in the upper part of the chart, corresponding to a stroke prevalence of 1. The red bubbles, on the other hand, are concentrated in the lower part of the chart, corresponding to a stroke prevalence of 0.

In terms of age, the blue bubbles are distributed across the entire age range from 20 to 80 years, with a slight concentration between 40 and 60 years. This suggests that although stroke can occur at any age, there is a tendency for higher prevalence in middle-aged individuals.

Regarding BMI, represented by the size of the bubbles, there does not seem to be a clear correlation between BMI and stroke prevalence. Bubbles of various sizes (indicative of variable BMI) are found both among those who have had a stroke (blue) and those who have not (red), indicating that BMI is not a primary indicator of stroke in this dataset.
...
data_bubble <- data.frame(
  ...
  blood_pressure = runif(100, 80, 180),
  glucose_level = runif(100, 70, 150)
)

# Creare il grafico a bolle con più parametri
p <- ggplot(data_bubble, aes(x = age, y = prevalence, size = bmi, color = gender, shape = factor(glucose_level > 100))) +
  geom_point(alpha = 0.7) +
  scale_size_continuous(range = c(3, 10)) +
  labs(title = "Bubble Chart with Multiple Parameters",
       x = "Age",
       y = "Stroke Prevalence",
       size = "BMI",
       color = "Gender",
       shape = "Glucose > 100 mg/dL") +
  theme_minimal()
...
The bubble chart shows the prevalence of stroke in relation to age, gender, BMI, and blood glucose levels.

Bubbles at the top indicate a stroke prevalence of 1, while those at the bottom indicate a prevalence of 0. Strokes are more common between the ages of 40 and 60. There is no significant difference between men (blue) and women (red) in terms of stroke prevalence. BMI, represented by the size of the bubbles, does not show a clear correlation with strokes.

Blood glucose levels, indicated by the shape of the bubbles (circles for glucose ≤ 100 mg/dL and triangles for glucose > 100 mg/dL), suggest a slight correlation: high glucose levels are more frequently associated with strokes.
...
# Creare un esempio di dataframe per illustrare
set.seed(123)
data <- data.frame(
  age = sample(20:80, 100, replace = TRUE),
  bmi = runif(100, 15, 40),
  glucose = runif(100, 70, 150),
  blood_pressure = runif(100, 80, 180),
  stroke = sample(c(0, 1), 100, replace = TRUE)
)
...
ggplot(data = melted_cor_matrix, aes(x = Var1, y = Var2, fill = value)) +
  ...
  labs(title = "Mappa di Calore delle Correlazioni",
       x = "Variables",
       y = "Variables")

```

The heat map shows the correlations between variables: stroke, blood pressure, glucose, BMI, and age. The colored cells indicate the strength and direction of the correlations, with red representing a strong positive correlation (+1), blue a strong negative correlation (-1), and shades of brown representing intermediate correlations.

Age shows a strong positive correlation with BMI and glucose, and a moderate correlation with blood pressure and stroke. BMI has a positive correlation with glucose and a lesser correlation with blood pressure and stroke. Glucose has a moderate correlation with stroke and a positive correlation with blood pressure and BMI. Blood pressure shows a moderate correlation with stroke and positive correlations with glucose and BMI. Stroke has moderate positive correlations with age, glucose, and blood pressure.

In summary, the heat map highlights that age, glucose, and blood pressure are positively correlated with stroke, suggesting that these factors could influence the likelihood of stroke. BMI shows correlations with other variables but a weaker correlation with stroke compared to other variables.
