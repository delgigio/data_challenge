...
```{r}
#Evoluzione del Tasso di Prevalenza dell'Ictus per Età
data_age <- data.frame(age_group = rep(c("0-20", "21-40", "41-60", "61-80", "80+"), 10),
                       prevalence = runif(50, 0, 0.1),
                       time = rep(1:10, each = 5))

# Creare il grafico a barre animato
p <- ggplot(data_age, aes(x = age_group, y = prevalence, fill = age_group)) +
  geom_bar(stat = "identity") +
  labs(title = "Evoluzione del Tasso di Prevalenza dell'Ictus per Età",
       x = "Fascia di Età",
       y = "Prevalenza") +
  theme_minimal() +
  theme(legend.position = "none")
...
p_anim <- p + transition_states(time, transition_length = 2, state_length = 1) +
  labs(title = 'Tempo: {closest_state}')
...
```
```{r}
#Evoluzione Temporale del Numero di Ictus
# Installare e caricare i pacchetti necessari
install.packages("gganimate")

library(gganimate)

# Supponiamo che i dati abbiano una colonna `date` per la data e `stroke` per i casi di ictus
data_time <- data %>%
  group_by(date, work_type) %>%
  summarize(stroke_count = sum(stroke))

# Creare il grafico animato
p <- ggplot(data_time, aes(x = date, y = stroke_count, color = work_type)) +
  geom_line() +
  labs(title = "Evoluzione Temporale del Numero di Ictus",
       x = "Data",
       y = "Numero di Ictus") +
  theme_minimal()
...
```{r}
#Distribuzione del BMI nel Tempo
# Assumiamo che i dati abbiano una colonna `date` per la data e `bmi` per il BMI
data_filtered <- data %>%
  filter(!is.na(bmi) & work_type != "")

# Creare il grafico a violino animato
p <- ggplot(data_filtered, aes(x = work_type, y = bmi, fill = work_type)) +
  geom_violin(trim = FALSE) +
  geom_boxplot(width = 0.1, fill = "white") +
  labs(title = "Distribuzione del BMI nel Tempo per Tipo di Lavoro",
       x = "Tipo di Lavoro",
       y = "BMI") +
  theme_minimal() +
  theme(legend.position = "none")
